...
var projectPath = "./docs-recipe-test.csproj";
    ...
    .Does<BuildData>((context, buildData) =>
{
    ...
    DotNetCoreRun(projectPath, new ProcessArgumentBuilder().Append(string.Format("preview --virtual-dir \"docs-recipe-test\" --output \"{0}\"", buildData.OutputDirectory)), settings);
});
    ...
    .Does<BuildData>((context, buildData) =>
{
    ...
    Kudu.Sync(buildData.OutputDirectory, publishFolder, new KuduSyncSettings {
        ArgumentCustomization = args=>args.Append("--ignore").AppendQuoted(".git;CNAME;_git2_*")
    });
